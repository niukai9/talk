...
export default {
    ...
    initWebGL(width, height) {
      ...
      const fragmentShaderSource = `
        ...
        void main() {
          ...
          float u = texture2D(u_uTexture, v_texCoord * 0.5).r - 0.5;
          float v = texture2D(u_vTexture, v_texCoord * 0.5).r - 0.5;
          float r = y + 1.402 * v;
  ...
  // 分离 Y 分量
  const ySize = width * height; // Y 分量大小
  const yData = yuvData.subarray(0, ySize); // Y 分量
