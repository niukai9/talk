<template>
  <div class="container">
    ...
    >
      ...
      <template v-slot="{ row }">
        <div>
          <span>【{{ row.txtlen }}】{{ row.front_txt }}</span>
        </div>
      </template>
    </el-table-column>
    </el-table>
    ...
    <div class="edit-area" style="width: 40%; height: calc(100vh - 50px);">
      <el-card class="edit-box" style="height: calc(100% - 20px); overflow-y: auto;">
        ...
        <el-button size="large" @click="saveChanges(1)">暂存</el-button>
        <el-button size="large" type="primary" @click="saveChanges(2)">完成</el-button>
      </div>
      </el-card>
      <!-- 保存按钮 -->
